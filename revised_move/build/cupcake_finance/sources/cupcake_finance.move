module cupcake_addr::cupcake_finance{
    ...
    struct ResourceAccountInfo has key{
        source: address,
    ...
    public entry fun deposit_in_resource_account(account: &signer, seeds:vector<u8>, amount_in_octa: u64){
        ...
        let resource_acct = ResourceAccountInfo{
            source: signer::address_of(account),
            resource_addr: vault_addr,
            resource_cap: vault_signer_cap
        };
        move_to(account,resource_acct);
        let apt = coin::withdraw<AptosCoin>(account,amount_in_octa);
        coin::deposit(vault_addr, apt);

    }

    public entry fun withdraw_from_resource_account(source_addr: address) acquires ResourceAccountInfo{

        let resource_acct = borrow_global_mut<ResourceAccountInfo>(source_addr);
        let balance = coin::balance<AptosCoin>(resource_acct.resource_addr);
        let resource_account_from_cap = account::create_signer_with_capability(&resource_acct.resource_cap);
        let apt = coin::withdraw<AptosCoin>(&resource_account_from_cap,balance);
        coin::deposit(source_addr,apt);        
    }
